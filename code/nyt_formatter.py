...
import re, os, codecs, sys, glob, random
...
###################
doca = eval(open('/Users/muhammedyidris/git/bootstrapping_classes/data/doca_protests.txt').read())
doca_test = random.sample(doca, 1250)
index = open('/Users/muhammedyidris/git/bootstrapping_classes/test/nyt/nyt_index.txt', 'a')
...
for year in years:
  ...
  for m in months:
    out = open('/Users/muhammedyidris/git/bootstrapping_classes/test/nyt/formatted_nyt/'+str(year)+'-'+str(m).zfill(2)+'.txt', 'w+')

    for d in days:
      path = '/Volumes/MYI_PASS/data/nyt_corpus/data/'+str(year)+'/'+str(m).zfill(2)+'/'+str(d).zfill(2)
      filelist = glob.glob( os.path.join(path, '*.xml') )

      for fullfile in filelist:
        filename = fullfile.split('data/')[-1]

        if filename in doca_test:
          continue
...
doca = open('/Users/muhammedyidris/git/bootstrapping_classes/test/doca_protest_test_cases.txt', 'w+')
print >> doca, doca_test
doca.close()

print 'Doca count: '+str(len(doca_test))
